...
int initialize(int audio,int socket,int defaults)
{
	...
	if (SDL_NumJoysticks() > 0) SDL_JoystickOpen(0); //deniska
	atexit(SDL_Quit);
	...
	if (audio!=0){
	    ...
	    #else
	    //if ( Mix_OpenAudio(44100, AUDIO_S16SYS,	2, 8192) < 0 ) {  deniska
...
/*
	...
	/*default settings*/
	if (defaults!=0){
		//setdisplay(640,480,0,0);
		setdisplay(480,272,0,0);  //deniska
		autoback(25);
