...
*/
	...
	rw=SDL_RWFromMem(fontbuf, bytenum);
	if (rw!=NULL)
	    font = TTF_OpenFontRW(rw,0,s);
	else
	    font=NULL;
...
int setfont(char *path)
{
	...
	if (fontfp!=NULL)
		fclose(fontfp);
	fontfp=fopen(fontPath, "rb");
	if (fontfp==NULL) {
		error_description="SDLengine error: could not open file ";
		error_type=1;
		SDLerr(stderr,"SDLengine error: could not open file  %s\n",SDL_GetError());
		return -1;

	}
	bytenum=fread(fontbuf,1, sizeof(fontbuf),  fontfp);
	if (bytenum<=0) {
		error_description="SDLengine error: could not read from file ";
		error_type=1;
		SDLerr(stderr,"SDLengine error: could not read from file  %s\n",SDL_GetError());
		fclose(fontfp);
		return -1;
	}
}
