...
void screendebug()
{
    ...
    screenopen(8,displaywidth(),150,0,displayheight()-160,displaywidth(),150,0);
    cls();
    ...
    pen(0);
    #if defined(PSP)
    prints("START Run|SELECT Step|HOME Exit");
    #else
    ...
    //F5 / START
    #if defined(PSP)
	while (getbuttonjoystick(0,11)==0) {
    #else
    while(key(286)==0 ){
    #endif
	//if (stopkey()==-1){
	if (getbuttonjoystick(0,12)==1){  //exit
	    terminate();
	    exit(2);
	}
	//F6
	#if defined(PSP)
	if(getbuttonjoystick(0,10)==1)break;
	#else
	if (getbuttonjoystick(12)==1)break;
	#endif

	//F4
	if (getbuttonjoystick(0,12)==1){
	    ...
	    pen(0);
	   // prints("F5 Run|F6 Step|F4 Watch|Ctrl+c Exit");
    	    prints("[START] Run|[SELECT] Step|[HOME] Watch");
	    paper(0);
	    pen(0xffffff);
	    screenswap();
    ...
    //F5
    #if defined(PSP)
    if(getbuttonjoystick(0,11)==1)debug=0; // step mode disabled
    #else
